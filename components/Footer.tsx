...
const Footer: React.FC<FooterProps> = ({ nodeCount, fillInfo, isRendering, renderProgress }) => {
  return (
    <footer className="xibalba-footer fixed bottom-0 left-0 right-0 h-12 bg-[var(--xibalba-grey-050)] border-t border-white/10 flex items-center justify-between font-black select-none z-40 px-6">
      <div className="flex items-center gap-8">
        <div className="flex items-center gap-3 group cursor-help">
          ...
          <span className="tracking-[0.15em] uppercase text-[var(--xibalba-text-300)]">Entities: <span className="text-white mono">{nodeCount}</span></span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="tracking-[0.15em] uppercase text-[var(--xibalba-text-300)]">Targeting: <span className="text-[var(--xibalba-text-100)] truncate max-w-[120px]">{fillInfo}</span></span>
        </div>
      </div>
      
      <div className="flex items-center gap-8 mono text-[var(--xibalba-text-300)]">
        <div className="flex items-center gap-4 text-[9px] opacity-70">
          <span className="flex items-center gap-1.5"><span className="size-1.5 rounded-none bg-green-500/50"></span>LAT: 8.4ms</span>
