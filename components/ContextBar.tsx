...
const ContextBar: React.FC<ContextBarProps> = ({ role, selectedLayerId, layers, cycles, verboseMode }) => {
  ...
  const renderNexusControls = () => (
    <div className="flex-1 flex items-center gap-10 h-full animate-in fade-in slide-in-from-left-4 duration-500">
      <div className="flex items-center gap-4">
        <span className="text-[8px] font-black text-obsidian-300 uppercase tracking-widest">{verboseMode ? 'Current_Schedule' : 'ACTIVE_CYCLE'}</span>
      ...
      <div className="flex items-center gap-4">
        <span className="text-[8px] font-black text-obsidian-300 uppercase tracking-widest">{verboseMode ? 'Search_Work' : 'QUERY_MANIFEST'}</span>
  ...
  const renderVectorControls = () => (
    <div className="flex-1 flex items-center h-full gap-10 animate-in fade-in duration-500">
      <div className="flex flex-col gap-1">
         <span className="text-[6px] font-black text-obsidian-300 uppercase tracking-widest px-1">BOOLEAN_FORGE</span>
      ...
      <div className="flex flex-col gap-1">
         <span className="text-[6px] font-black text-obsidian-300 uppercase tracking-widest px-1">SPATIAL_SYNC</span>
    ...
    <div className="h-14 bg-obsidian-850 border-b border-white/[0.02] flex items-center z-[300] select-none relative grain-layer grain-3">
      ...
      <div className="flex-1 h-full flex items-center px-10 overflow-hidden">
        <div className="flex flex-col mr-12 shrink-0">
            <span className="text-[6px] font-mono text-obsidian-300 uppercase tracking-widest italic opacity-40">WORKSPACE_CORE</span>
            <span className="text-[12px] font-black text-obsidian-100 uppercase tracking-[0.2em] italic leading-tight">{role}</span>
        </div>
      ...
      {(role !== WorkspaceRole.PROJECT_NEXUS && selectedLayerId) && (
        <div className="flex items-center gap-8 border-l border-white/[0.01] px-10 h-full animate-in fade-in slide-in-from-right-4 duration-500">
           <div className="flex flex-col items-end gap-1">
              <span className="text-[6px] font-black text-obsidian-300 uppercase tracking-widest opacity-40 italic">ENTITY_SIG</span>
              <div className="flex items-center gap-3">
                 ...
                 <span className="text-[9px] font-mono text-primary font-bold tabular-nums italic">{selectedLayer?.nodes.length} PTS</span>
              </div>
           </div>
        </div>
      )}
