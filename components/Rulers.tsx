...
const Rulers: React.FC<RulersProps> = ({ zoom, pan, onAddGuide }) => {
  ...
  return (
    <>
      {/* Zero Point - Must be first, highest z-index */}
      <div className="absolute top-0 left-0 w-8 h-8 bg-[var(--xibalba-grey-100)] border-r border-b border-white/10 z-[80] flex items-center justify-center">
        <span className="material-symbols-outlined text-[14px] text-[var(--xibalba-text-100)]">straighten</span>
      </div>

      {/* Horizontal Ruler - Top of canvas, starts after zero point, overlays canvas */}
      <div 
        className="absolute top-0 left-8 right-0 h-8 bg-[var(--xibalba-grey-050)] border-b border-white/10 z-[70] cursor-ns-resize overflow-hidden"
        onClick={(e) => {
      ...
      {/* Vertical Ruler - Left side of canvas, starts below zero point, overlays canvas */}
      <div 
        className="absolute top-8 left-0 bottom-0 w-8 bg-[var(--xibalba-grey-050)] border-r border-white/10 z-[70] cursor-ew-resize overflow-hidden"
        onClick={(e) => {
          const rect = e.currentTarget.getBoundingClientRect();
